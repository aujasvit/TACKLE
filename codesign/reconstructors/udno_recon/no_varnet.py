import math
from typing import Optional, Union, Iterable, Tuple, List, Literal
# import fastmri
import torch
...
def sens_reduce(k: torch.Tensor, sens_maps: torch.Tensor) -> torch.Tensor:
    ...
    """
    return _complex_mul(_ifft2c(k), _complex_conj(sens_maps)).sum(
        dim=1, keepdim=True
    )
...
class SensitivityModel(nn.Module):
    ...
    def forward(
        ...
        return self.divide_root_sum_of_squares(
            self.batch_chans_to_chan_dim(self.norm_undno(images), batches)
        )
...
#         or (isinstance(num_low_frequencies, int) and num_low_frequencies == 0) \
#         or (isinstance(num_low_frequencies, Iterable) and any(n == 0 for n in num_low_frequencies)):
...
#             )
#             masked_kspace = _batched_mask_center(
#                 masked_kspace, pad, pad + num_low_freqs
#             )

#         # convert to image space
#         images, batches = chans_to_batch_dim(_ifft2c((torch.view_as_real(masked_kspace))))
